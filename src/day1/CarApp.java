 ...
 * 	반환값이 없다.
 * 	매소드 이름은 changeCurrentSpeed다.
 * 	메소드 실행시 전달할 인자값은 정수형의 속도값이다.
 *  메소드의 실행방법 : car.changeCurrentSpeed(50);
 * 
 * speedUp(): void - Car
 *  반환값이 없다.
 * 	매소드 이름은 speedUp이다.
 * 	메소드 실행시 전달할 인자값은 없다.
 *  메소드의 실행방법 : car.speedUp();
 * 
 * printCurrentSpeed(): void - Car
 *  반환값이 없다.
 * 	매소드 이름은 printCurrentSpeed다.
 * 	메소드 실행시 전달할 인자값은 없다.
 *  메소드의 실행방법 : car.printCurrentSpeed();
 * 
 * getCurrentSpeed(): int - Car
 *  반환값이 있다. 정수타입의 값을 반환한다. 즉, 정수타입의 변수를 선언해서 값을 저장한다.
 * 	매소드 이름은 getCurrentSpeed다.
 * 	메소드 실행시 전달할 인자값은 없다.
 *  메소드의 실행방법 : int speed = car.getCurrentSpeed();
 */